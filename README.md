...
- 未配置的目录逐层回退，最终使用 default。目录级只有两态：「配了字段」与「未配置」——
  空数组（`目录: []`）等价于未配置，会继续向上继承（不存在「显式中断聚合」这一态）。
  「整个域不做字段聚合」仍然成立：把 `branches.default` 留空（或不写 `branches`）即可。
- 目录 key 统一为 Quartz slug 路径，规范化后冲突直接报错。`/` 代表根目录。
- minGroupSize 默认 2，允许 1。graph-pro 在目录分支当前层只要有一类达到阈值，便统一聚合全部类别；否则尝试下一条规则，最终整体散开。本插件不创建图节点。
  取 1 表示「每个取值都成组」——需要“每个维度值都有聚合节点、都能进维度值页”时使用（与维度页的全量出页口径一致）；目录层同理，单成员目录也会成组。
- 新配置缺失时不输出产物；新配置无效时抛错，禁止静默回退。
- 规则只有 `folder` 与 `field` 两种；日期维度由字段值承担（内容侧把值写成年 / 年月），插件不做隐式日期解析，也不再接受 `granularity`（出现即报 unknown key）。
